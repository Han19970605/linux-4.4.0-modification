...
unsigned int tcp_current_mss(struct sock *sk)
{
	...
	/* record the max mss can be used */
	u32 mss_temp = mss_now;
	/**
	 * get the predicted value
	 * adjust the cwnd 
	 * */
	char *path = "/bin/MLmodule";
	char *argv[] = {path, NULL};
	char *envp[] = {
		"HOME=/",
	...
	extern int my_proc_predict(int, int, int, int, int, int, int);
	/* parameters needed here */
	int mss_predict = my_proc_predict();
	if (mss_predict <= mss_temp)
		mss_now = mss_predict;
...
bool tcp_may_send_now(struct sock *sk)
{
	...
	return skb &&
		   tcp_snd_test(sk, skb, tcp_current_mss(sk),
						(tcp_skb_is_last(sk, skb) ? tp->nonagle : TCP_NAGLE_PUSH));
}
...
/**
 * \param sk 套接字
 * \param mss_now 当前有效的MSS大小
 * \param nonagle 是否开启nagle算法
 * \param push_one 当push_one大于0时，最多发送一个包
*/
static bool tcp_write_xmit(struct sock *sk, unsigned int mss_now, int nonagle,
						   int push_one, gfp_t gfp)
{
...
static void tcp_connect_init(struct sock *sk)
{
	...
	/* adv_mss设置为user_mss和路由表中查询到的最小值 */
	if (tp->rx_opt.user_mss && tp->rx_opt.user_mss < tp->advmss)
		tp->advmss = tp->rx_opt.user_mss;

	/* 将sk中的rcv_mss设置为advmss和mss_cache中较小的一个 */
